...
export const NOIChart: React.FC<NOIChartProps> = ({
  withoutUpgrade,
  withUpgrade,
  events,
  valueType,
  ...
  useEffect(() => {
    console.log('NOIChart useEffect:', {
    ...
    if (!svgRef.current || !withoutUpgrade.length || !withUpgrade.length) {
      console.log('Early return from NOIChart useEffect');
      return;
    }
    ...
    <div className="relative">
        ...
        <div className="absolute top-4 left-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-3 shadow-lg">
          ...
          <div className="text-xs space-y-1">
            <div className="flex justify-between gap-4">
              ...
              <span className="font-medium text-gray-900 dark:text-gray-100">${hoveredPoint.without.toLocaleString()}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span className="text-gray-600 dark:text-gray-300">With upgrade:</span>
              <span className="font-medium text-gray-900 dark:text-gray-100">${hoveredPoint.with.toLocaleString()}</span>
            </div>
            <div className="flex justify-between gap-4 pt-1 border-t">
              ...
              <span className={`font-medium ${hoveredPoint.difference >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ${Math.abs(hoveredPoint.difference).toLocaleString()}
              </span>
            </div>
          </div>
        </div>
      ...
      {hoveredEvent && (
        <div
          ...
          style={{
            left: `${120 + (hoveredEvent.year - 2025) * 35}px`,
            top: '60px',
